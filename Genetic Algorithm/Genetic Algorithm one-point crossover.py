...
def crossing(chromosome1, chromosome2):
    while True:
        ...
        new_chromosome1 = chromosome1[:i] + chromosome2[i:]
        new_chromosome2 = chromosome2[:i] + chromosome1[i:]
...
def genetic_algorithm(generation, historical_prices, capital):
    ...
    for gen in range(generation):
        ...
        for i in range(len(parent_arr)):
            for j in range(i + 1, len(parent_arr)):
                offspring1, offspring2 = crossing(parent_arr[i], parent_arr[j])
                offsprings.append(offspring1)
                offsprings.append(offspring2)

        for i in range(len(offsprings)):
            mut = mutation(offsprings[i])
            offsprings[i] = mut
...
result = genetic_algorithm(10, [-1.2, 3.4, -0.8, 2.1, -2.5, 1.7, -0.3, 5.8, -1.1, 3.5], 1000)
result_dic = {"stop_loss": int(result[0][:2]), "take_profit": int(result[0][2:4]), "trade_size": int(result[0][4:])}
print(f"Best_strategy: {result_dic} \nFinal_profit : {round(result[1], 2)}")
